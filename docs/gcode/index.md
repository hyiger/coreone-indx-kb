...
behavior that is not obvious from the slicer.
...
you expect can silently change behavior for every tool at once.

Several reported INDX faults trace back to G-code and slicer behavior rather than
...
- Firmware behavior: which commands are honoured, which are ignored, and which
...
from, and says whether it is the shipped default or somebody's customization — those are
