...
!!! warning "Two owners, one thread, and one teardown"
    ...
    evidence for that unit and says nothing yet about how common the defect is.
    `provisional`.
...
Once the tool was loose, the first owner found the printer stuck retrying the dock with
only Retry and Abort on offer. After a reboot it reported a tool present without
knowing which one, and went straight back to trying to dock it — which kept them away
from the "Release Stuck Tool" menu item that might have freed it.
...
toolhead apart, or a replacement toolhead.
...
`provisional` — two owners in one thread, and a physical cause found on one machine
only.
...
The teardown post was still awaiting moderation on the forum when this page was
written, so the link may not show it to every reader yet. The owner's video and a set
